...
import { Stars, DarkCard, Btn, Badge, MiniChart } from "../components/RiskDashboard";

// ── Floating metric card helper ───────────────────────────────────────────────
function FloatCard({ children, style }) {
  return (
    <div style={{ background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 16, padding: "10px 14px", boxShadow: "0 20px 60px rgba(0,0,0,0.5)", backdropFilter: "blur(10px)", ...style }}>
      {children}
    </div>
  );
}
    ...
    <div style={{ background: T.bg, color: T.cream, fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <section style={{ position: "relative", minHeight: "92vh", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", background: "radial-gradient(ellipse 80% 60% at 50% -10%, rgba(232,64,64,0.15) 0%, transparent 60%), #0a0a0a" }}>
        ...
        <div style={{ position: "relative", zIndex: 2, textAlign: "center" }}>
          <div style={{ width: 220, margin: "0 auto 0", background: "#1c1c1e", borderRadius: 36, padding: "16px 14px 24px", border: "1px solid rgba(255,255,255,0.12)", boxShadow: "0 40px 100px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.1)", animation: "float 6s ease-in-out infinite" }}>
            ...
            <div style={{ background: T.cream, borderRadius: 24, padding: "18px 14px", minHeight: 280 }}>
              ...
              <div style={{ marginTop: 14, background: "#e84040", borderRadius: 12, padding: "10px 0", textAlign: "center", fontSize: 12, fontWeight: 700, color: "white" }}>Start Assessment →</div>
            </div>
          </div>
      ...
      <section style={{ background: "#0f0f0f", padding: "80px 60px", borderRadius: "24px 24px 0 0" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <div style={{ display: "flex", gap: 60, alignItems: "flex-start" }}>
            <div style={{ flex: 1 }}>
              <h2 style={{ fontFamily: "'Instrument Serif',serif", fontSize: 36, fontWeight: 400, letterSpacing: -1.2, lineHeight: 1.15, color: T.cream, marginBottom: 16 }}>
                Your ultimate cognitive health platform, packed with features
              </h2>
            </div>
            <div style={{ flex: 1, paddingTop: 6 }}>
              ...
              <Btn variant="cream" onClick={() => setView("login")}>↓ Start now</Btn>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 48 }}>
            ...
            <DarkCard style={{ padding: 28 }}>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
                <div style={{ width: 80, height: 80, borderRadius: "50%", background: "radial-gradient(circle at 35% 35%, #ffd700, #b8860b)", boxShadow: "0 0 40px rgba(255,215,0,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 36 }}>⬡</div>
              </div>
              <div style={{ fontSize: 22, fontWeight: 700, color: T.cream, marginBottom: 10 }}>Effortless Assessments</div>
              <div style={{ color: T.creamFaint, fontSize: 14, lineHeight: 1.7 }}>Complete cognitive tests with a guided, user-friendly interface.</div>
            </DarkCard>
            <DarkCard style={{ padding: 28 }}>
      ...
      <section style={{ background: "#0f0f0f", padding: "80px 60px", textAlign: "center" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <h2 style={{ fontFamily: "'Instrument Serif',serif", fontSize: 38, fontWeight: 400, letterSpacing: -1.2, color: T.cream, marginBottom: 10 }}>
            Explore, assess, and track<br /><em style={{ color: T.creamDim }}>seamlessly in the cognitive ecosystem.</em>
          </h2>
          ...
          <div style={{ display: "flex", gap: 60, alignItems: "center", textAlign: "left" }}>
            <div style={{ flex: 1 }}>
      ...
      <section style={{ background: "#0e0a18", padding: "80px 60px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", bottom: -100, left: "50%", transform: "translateX(-50%)", width: 600, height: 400, background: "radial-gradient(ellipse 60% 80% at 50% 100%, rgba(232,130,40,0.18) 0%, transparent 70%)", pointerEvents: "none" }} />
        <div style={{ maxWidth: 1000, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <p style={{ color: T.creamFaint, fontSize: 14, marginBottom: 56 }}>From testing to tracking, access all 6 cognitive screening tools, all in one secure place.</p>
          <div style={{ display: "flex", gap: 24, justifyContent: "center", marginBottom: 32 }}>
            {[{ label: "Speech AI", bg: "linear-gradient(135deg,#7b2020,#e84040)", icon: "🎙️", rot: "-8deg" }, { label: "MemoryTest", bg: "linear-gradient(135deg,#1a3a8b,#4060d0)", icon: "🧠", rot: "0deg" }, { label: "ReactionX", bg: "linear-gradient(135deg,#1a6040,#28c070)", icon: "⚡", rot: "8deg" }].map((t) => (
              <div key={t.label} onClick={() => setView("login")} style={{ width: 140, height: 160, borderRadius: 24, background: t.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, cursor: "pointer", transform: `rotate(${t.rot})`, boxShadow: "0 16px 50px rgba(0,0,0,0.6)", border: "1px solid rgba(255,255,255,0.12)", transition: "transform 0.25s" }}
                ...
                <div style={{ fontSize: 12, fontWeight: 700, color: "rgba(255,255,255,0.9)" }}>{t.label}</div>
              </div>
            ))}
          </div>
          <p style={{ color: T.creamDim, fontSize: 13, marginBottom: 48, fontStyle: "italic" }}>From speech biomarkers to reaction timing — all your cognitive tools in one secure place.</p>
          <div style={{ display: "flex", gap: 24, justifyContent: "center" }}>
            {[{ label: "Progress", bg: "linear-gradient(135deg,#2a6020,#50b030)", icon: "📈", rot: "-6deg" }, { label: "DocPortal", bg: "linear-gradient(135deg,#5a2080,#9040d0)", icon: "🩺", rot: "0deg" }, { label: "Reports", bg: "linear-gradient(135deg,#1a4050,#2090b0)", icon: "📄", rot: "6deg" }].map((t) => (
              <div key={t.label} onClick={() => setView("login")} style={{ width: 140, height: 160, borderRadius: 24, background: t.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, cursor: "pointer", transform: `rotate(${t.rot})`, boxShadow: "0 16px 50px rgba(0,0,0,0.6)", border: "1px solid rgba(255,255,255,0.12)", transition: "transform 0.25s" }}
                onMouseEnter={(e) => (e.currentTarget.style.transform = "scale(1.06) rotate(0deg)")}
                onMouseLeave={(e) => (e.currentTarget.style.transform = `rotate(${t.rot})`)}>
                <div style={{ fontSize: 40 }}>{t.icon}</div>
          ...
          <div style={{ marginTop: 32 }}><Btn variant="ghost" style={{ borderRadius: 12 }}>↓ More</Btn></div>
        </div>
      </section>
      ...
      <section style={{ position: "relative", background: T.bg, padding: "120px 60px", textAlign: "center", overflow: "hidden" }}>
        <Stars count={90} />
        <div style={{ position: "absolute", bottom: -60, left: "50%", transform: "translateX(-50%)", width: 700, height: 350, background: "radial-gradient(ellipse 70% 100% at 50% 100%, rgba(180,100,20,0.22) 0%, transparent 70%)", pointerEvents: "none" }} />
      ...
      <footer style={{ background: T.bg, borderTop: `1px solid ${T.cardBorder}`, padding: "40px 60px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 40 }}>
        <div>{["Home", "Guides", "Support"].map((l) => <button key={l} style={{ display: "block", background: "none", border: "none", color: T.creamFaint, fontSize: 13, cursor: "pointer", marginBottom: 8, fontFamily: "'DM Sans',sans-serif" }}>{l}</button>)}</div>
        <div>
